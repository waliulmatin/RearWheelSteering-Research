 ...
 * Version: 2.1
 * Update log: Further improvements for stability
 */
...
int A_SIG=0, B_SIG=1;                        //Encoder signal variables
 ...
 //Serial.println(accPos);
  ...
  if((pot >= 525) && (pot <= 811)){           //Sets up condition so device will only control the motor when the steering wheel is in the desired range
